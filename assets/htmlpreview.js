/* Adapted from https://github.com/htmlpreview/htmlpreview.github.com */
...
var HTMLPreview = {
	...
	file: function() {
		return "https://github.com/LIJI32/SameBoy/blob/automation_results/results.html";
	},
	...
	loadHTML: function(data) {
		...
		&& data.query.diagnostics.redirect) {
			...
			HTMLPreview.content = data.query.results.resources.content.replace(/<head>/i, '<head><link rel="stylesheet" href="/assets/iframe.css"><base href="' + HTMLPreview.raw() + '">').replace(/<script( type=["'](text|application)\/javascript["'])?/gi, '<script type="text/htmlpreview"').replace(/<\/body>/i, '<script src="//' + location.hostname + '/htmlpreview.min.js"></script><script>HTMLPreview.replaceAssets();</script></body>').replace(/<\/head>\s*<frameset/gi, '<script src="//' + location.hostname + '/htmlpreview.min.js"></script><script>document.addEventListener("DOMContentLoaded",HTMLPreview.replaceAssets,false);</script></head><frameset'); //Add <base> just after <head> and inject <script> just before </body> or </head> if <frameset>
			setTimeout(function() {
				document.open();
